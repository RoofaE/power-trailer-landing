...
var calcData = [
  ...
  {
    category: "Security & Remote Monitoring",
    items: [
...
if (calcCategoriesEl) {
  ...
  calcData.forEach(function (cat, catIndex) {
    ...
    calcHtml +=
      ...
      cat.category +
      '<span class="calc-toggle-icon">+</span></button>';
  ...
  var calcCheckboxes = document.querySelectorAll(
  ...
  calcModeButtons.forEach(function (btn) {
    btn.addEventListener("click", function () {
      calcModeButtons.forEach(function (b) {
  ...
  function updateCalculator() {
    ...
    if (netLoad <= 0 || batteryHours >= 12) {
      ...
      statusText =
        "Limited runtime for this load. Consider reducing usage or contact Flaman.";
